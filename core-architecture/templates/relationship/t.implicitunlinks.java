.if (is_subsuper)
void unrelate_${rel_name}(${self.part_name}.R${self.num}Subtype form) {
.else
void unrelate_${rel_name}(${self.form_name} form) {
.end if
...
void unrelate_${rel_name}(${self.part_name} part) {
...
void unrelate_${rel_name}(Iterable<?> insts) {
    insts.forEach(inst -> {
.if (is_subsuper)
