...
.if ( returns_set )
        for ( IRelationship r$t{self.rel_num} : R$t{self.rel_num}set ) {
            $l{type_name}.add( population().${target_class_name}instances().getByInstanceId( ((${self.relationship_cast})r$t{self.rel_num}).get$c{self.dst_class}() ) );
        }
        return $l{type_name}.toImmutableSet();
.else
        if ( 1 == R$t{self.rel_num}set.size() ) {
            return (${type_name})population().${target_class_name}instances().getByInstanceId( ((${self.relationship_cast})R$t{self.rel_num}set.iterator().next()).get$c{self.dst_class}() );
        }
