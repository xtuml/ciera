...
public abstract class StateMachine<T extends IModelInstance<T,C>, C extends IComponent<C>> implements IStateMachine<T,C>, IInstanceActionHome<T,C> {
	...
	public void transition(IEvent event) throws XtumlException {
		...
		if ( currentState >= 0 && currentState < sem.length && event.getId() >= 0 && event.getId() < sem[currentState].length ) {
		...
		if ( entry instanceof CantHappen ) {
            throw new CantHappenException(getClassName() + ": Event '" + event.getName() + "' cannot happen in state '" + getStateName(currentState) + "'");
		}
		else if ( entry instanceof Ignore ) {
			/* do nothing */
		}
		else if ( entry instanceof ITransition ){
			ITransition txn = (ITransition)entry;
			int prevState = currentState;
            currentState = txn.execute(event);
            System.out.printf("TXN: %-15s: %-50s -> %-50s\n", getClassName(), getStateName(prevState), getStateName(currentState));
		}
		else {
		...
		for ( Field field : fields ) {
			try {
				if ( Modifier.isPublic(field.getModifiers()) && Modifier.isStatic(field.getModifiers()) &&
			         Modifier.isFinal(field.getModifiers()) && field.getType() == Integer.TYPE && field.getInt(this) == state ) {
					name = field.getName();
				}
