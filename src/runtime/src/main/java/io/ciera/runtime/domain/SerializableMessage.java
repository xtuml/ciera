...
public class SerializableMessage extends Message {
    ...
    public SerializableMessage(Message message) {
        this(message.getMessageHandle(), message.getId(), message.getName(), message.getParameterData(),
                message instanceof SerializableMessage ? ((SerializableMessage) message).parametersAreValid : true);
    }
    ...
    public static SerializableMessage fromString(String s, Class<?> iface) {
        try {
            ...
            if (iface != null) {
                try {
                    return new SerializableMessage(messageHandle, id, name,
    ...
    private static Map<Integer, Map<String, Class<?>>> getTypeMap(Class<?> cls) {
        ...
        Stream.of(cls.getDeclaredFields())
                .filter(f -> Modifier.isStatic(f.getModifiers()) && int.class.equals(f.getType())
                        && f.getAnnotation(Message.Names.class) != null && f.getAnnotation(Message.Types.class) != null)
                .forEach(f -> {
                    ...
                    String[] names = f.getAnnotation(Message.Names.class).names();
                    Class<?>[] types = f.getAnnotation(Message.Types.class).types();
