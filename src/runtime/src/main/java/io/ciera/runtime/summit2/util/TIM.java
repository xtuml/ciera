...
import io.ciera.runtime.summit2.types.Date;
...
public class TIM extends Utility {
    ...
    public TimeStamp set_time(final int year, final int month, final int day, final int hour, final int minute, final int second, final int microsecond) {
        Calendar cal = Calendar.getInstance();
        cal.setTimeZone(TimeZone.getTimeZone("UTC"));
        cal.set(year, month - 1, day, hour, minute, second);
        cal.set(Calendar.MILLISECOND, 0);
    ...
    public TimerHandle timer_start_recurring(final EventHandle event_inst, final int microseconds) {
        ...
        if (event != null) {
            EventTarget target = getDomain().getTarget(event.getTargetHandle());
            return getContext().scheduleEvent(event, target, new Duration(microseconds * 1000l), new Duration(microseconds * 1000l));
        }
        return null;
    }
